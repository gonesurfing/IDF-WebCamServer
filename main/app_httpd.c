...
void enable_led(bool en) {  // Turn LED On or Off
  ...
  //ledcWrite(LED_LEDC_GPIO, duty); EWT comment out
...
#endif
  ESP_LOGI(TAG, "JPG: %uB %lims", (fb_len), (uint32_t)((fr_end - fr_start) / 1000));
  return res;
}

static esp_err_t stream_handler(httpd_req_t *req) {
  ESP_LOGI(TAG, "Stream handler");
  camera_fb_t *fb = NULL;
  ...
  while (true) {
    ...
    ESP_LOGI(TAG,
      "MJPG: %liB %lims (%.1ffps), AVG: %lims (%.1ffps)", (uint32_t)(_jpg_buf_len), (uint32_t)frame_time, 1000.0 / (uint32_t)frame_time, avg_frame_time,
      1000.0 / (double)avg_frame_time
    );
#endif
}
...
static esp_err_t greg_handler(httpd_req_t *req) {
  ESP_LOGI(TAG, "Get Register handler");
  char *buf = NULL;
  char _reg[32];
  char _mask[32];
